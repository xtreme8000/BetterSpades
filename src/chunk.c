		...
		for(int z = worker->chunk_y; z < worker->chunk_y + CHUNK_SIZE; z++) {
			for(int y = 0; y < map_size_y; y++) {
				...
				if(map_colors[index] != 0xFFFFFFFF) {
					...
					if((x == 0 && map_colors[map_size_x - 1 + (y * map_size_z + z) * map_size_x] == 0xFFFFFFFF)
					   || (x > 0 && map_colors[index - 1] == 0xFFFFFFFF)) {
						size++;
					}
					if((x == map_size_x - 1 && map_colors[(y * map_size_z + z) * map_size_x] == 0xFFFFFFFF)
					   || (x < map_size_x - 1 && map_colors[index + 1] == 0xFFFFFFFF)) {
						size++;
					}
					if((z == 0 && map_colors[x + (y * map_size_z + map_size_z - 1) * map_size_x] == 0xFFFFFFFF)
					   || (z > 0 && map_colors[x + (y * map_size_z + z - 1) * map_size_x] == 0xFFFFFFFF)) {
						size++;
					}
					if((z == map_size_z - 1 && map_colors[x + (y * map_size_z) * map_size_x] == 0xFFFFFFFF)
					   || (z < map_size_z - 1 && map_colors[x + (y * map_size_z + z + 1) * map_size_x] == 0xFFFFFFFF)) {
						size++;
					}
				}
			}
				...
				if(col != 0xFFFFFFFF) {
						...
						if(checked_voxels2[0][y + (x - worker->chunk_x) * map_size_y] == 0) {
							...
							for(int a = 0; a < map_size_y - y; a++) {
								if(map_get(x, y + a, z) == col
								   && checked_voxels2[0][y + a + (x - worker->chunk_x) * map_size_y] == 0
							...
							for(int b = 1; b < (worker->chunk_x + CHUNK_SIZE - x); b++) {
								...
								for(a = 0; a < len_y; a++) {
									if(map_get(x + b, y + a, z) != col
									   || checked_voxels2[0][y + a + (x + b - worker->chunk_x) * map_size_y] != 0
					...
					if((z == map_size_z - 1 && map_isair(x, y, 0)) || (z < map_size_z - 1 && map_isair(x, y, z + 1))) {
						if(checked_voxels2[1][y + (x - worker->chunk_x) * map_size_y] == 0) {
							...
							for(int a = 0; a < map_size_y - y; a++) {
								if(map_get(x, y + a, z) == col
								   && checked_voxels2[1][y + a + (x - worker->chunk_x) * map_size_y] == 0
							...
							for(int b = 1; b < (worker->chunk_x + CHUNK_SIZE - x); b++) {
								...
								for(a = 0; a < len_y; a++) {
									if(map_get(x + b, a + a, z) != col
									   || checked_voxels2[1][y + a + (x + b - worker->chunk_x) * map_size_y] != 0
				...
				if(col != 0xFFFFFFFF) {
					...
					if((x == 0 && map_isair(map_size_x - 1, y, z)) || (x > 0 && map_isair(x - 1, y, z))) {
						if(checked_voxels2[0][y + (z - worker->chunk_y) * map_size_y] == 0) {
							...
							for(int a = 0; a < map_size_y - y; a++) {
								if(map_get(x, y + a, z) == col
								   && checked_voxels2[0][y + a + (z - worker->chunk_y) * map_size_y] == 0
							...
							for(int b = 1; b < (worker->chunk_y + CHUNK_SIZE - z); b++) {
								...
								for(a = 0; a < len_y; a++) {
									if(map_get(x, y + a, z + b) != col
									   || checked_voxels2[0][y + a + (z + b - worker->chunk_y) * map_size_y] != 0
					...
					if((x == map_size_x - 1 && map_isair(0, y, z)) || (x < map_size_x - 1 && map_isair(x + 1, y, z))) {
						if(checked_voxels2[1][y + (z - worker->chunk_y) * map_size_y] == 0) {
							...
							for(int a = 0; a < map_size_y - y; a++) {
								if(map_get(x, y + a, z) == col
								   && checked_voxels2[1][y + a + (z - worker->chunk_y) * map_size_y] == 0
							...
							for(int b = 1; b < (worker->chunk_y + CHUNK_SIZE - z); b++) {
								...
								for(a = 0; a < len_y; a++) {
									if(map_get(x, y + a, z + b) != col
									   || checked_voxels2[1][y + a + (z + b - worker->chunk_y) * map_size_y] != 0
				...
				if(col != 0xFFFFFFFF) {
						...
						if(checked_voxels[0][(x - worker->chunk_x) + (z - worker->chunk_y) * CHUNK_SIZE] == 0) {
							...
							for(int a = 0; a < (worker->chunk_x + CHUNK_SIZE - x); a++) {
								if(map_get(x + a, y, z) == col
								   && checked_voxels[0][(x + a - worker->chunk_x) + (z - worker->chunk_y) * CHUNK_SIZE]
									   == 0
							...
							for(int b = 1; b < (worker->chunk_y + CHUNK_SIZE - z); b++) {
								...
								for(a = 0; a < len_x; a++) {
									if(map_get(x + a, y, z + b) != col
									   || checked_voxels[0][(x + a - worker->chunk_x)
															+ (z + b - worker->chunk_y) * CHUNK_SIZE]
										   != 0
							...
							for(int b = 0; b < len_z; b++)
								for(int a = 0; a < len_x; a++)
									checked_voxels[0]
												  [(x + a - worker->chunk_x) + (z + b - worker->chunk_y) * CHUNK_SIZE]
										= 1;
					...
					if(y > 0 && map_isair(x, y - 1, z)) {
						if(checked_voxels[1][(x - worker->chunk_x) + (z - worker->chunk_y) * CHUNK_SIZE] == 0) {
							...
							for(int a = 0; a < (worker->chunk_x + CHUNK_SIZE - x); a++) {
								if(map_get(x + a, y, z) == col
								   && checked_voxels[1][(x + a - worker->chunk_x) + (z - worker->chunk_y) * CHUNK_SIZE]
									   == 0
							...
							for(int b = 1; b < (worker->chunk_y + CHUNK_SIZE - z); b++) {
								...
								for(a = 0; a < len_x; a++) {
									if(map_get(x + a, y, z + b) != col
									   || checked_voxels[1][(x + a - worker->chunk_x)
															+ (z + b - worker->chunk_y) * CHUNK_SIZE]
										   != 0
		...
		for(int x = worker->chunk_x; x < worker->chunk_x + CHUNK_SIZE; x++) {
			...
			for(int y = 0; y < map_size_y; y++) {
				...
				if(col != 0xFFFFFFFF) {
					...
					if((x == 0 && map_colors[map_size_x - 1 + (y * map_size_z + z) * map_size_x] == 0xFFFFFFFF)
					   || (x > 0 && map_colors[x + (y * map_size_z + z) * map_size_x - 1] == 0xFFFFFFFF)) {
						if((size + 1) > worker->mem_size) {
							worker->mem_size += 512; // allow for 512 more quads
							worker->vertex_data = realloc(worker->vertex_data, worker->mem_size * VERTEX_ADVANCE);
							CHECK_ALLOCATION_ERROR(worker->vertex_data)
					...
					if((x == map_size_x - 1 && map_colors[(y * map_size_z + z) * map_size_x] == 0xFFFFFFFF)
					   || (x < map_size_x - 1 && map_colors[x + (y * map_size_z + z) * map_size_x + 1] == 0xFFFFFFFF)) {
						if((size + 1) > worker->mem_size) {
							worker->mem_size += 512; // allow for 512 more quads
							worker->vertex_data = realloc(worker->vertex_data, worker->mem_size * VERTEX_ADVANCE);
							CHECK_ALLOCATION_ERROR(worker->vertex_data)
...
void chunk_update_all() {
	for(int j = 0; j < chunk_enabled_cores; j++) {
		...
		if(chunk_workers[j].state == CHUNK_WORKERSTATE_FINISHED) {
			...
			glx_displaylist_update(&chunks[chunk_workers[j].chunk_id].display_list, chunk_workers[j].data_size,
								   GLX_DISPLAYLIST_NORMAL, chunk_workers[j].color_data, chunk_workers[j].vertex_data,
								   NULL);

			int tex_data[CHUNK_SIZE * CHUNK_SIZE];
			for(int y = 0; y < CHUNK_SIZE; y++) {
				for(int x = 0; x < CHUNK_SIZE; x++) {
					tex_data[x + y * CHUNK_SIZE] = ((
						int*)map_minimap)[x + chunk_workers[j].chunk_x + (y + chunk_workers[j].chunk_y) * map_size_x];
				}
			}
			...
			glTexSubImage2D(GL_TEXTURE_2D, 0, chunk_workers[j].chunk_x, chunk_workers[j].chunk_y, CHUNK_SIZE,
							CHUNK_SIZE, GL_RGBA, GL_UNSIGNED_BYTE, tex_data);
			pthread_mutex_unlock(&chunk_minimap_lock);
