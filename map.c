...
void map_vxl_setgeom(int x, int y, int z, unsigned int t, unsigned int* map) {
	...
	/*map[x+1+((map_size_y-1-z)*map_size_z+y)*map_size_x] = t;
	map[x+((map_size_y-1-z)*map_size_z+y+1)*map_size_x] = t;
	map[x+1+((map_size_y-1-z)*map_size_z+y+1)*map_size_x] = t;
	
	map[x+((map_size_y-1-z-1)*map_size_z+y)*map_size_x] = t;
	map[x+1+((map_size_y-1-z-1)*map_size_z+y)*map_size_x] = t;
	map[x+((map_size_y-1-z-1)*map_size_z+y+1)*map_size_x] = t;
	map[x+1+((map_size_y-1-z-1)*map_size_z+y+1)*map_size_x] = t;*/
}
